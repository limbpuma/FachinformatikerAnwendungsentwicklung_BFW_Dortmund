	...
	public static void main( String[] args ) throws NumberFormatException, IOException
	{
		...
		for ( int index = 0; index < data.length; index++ )// Hemos quitado la condicion ! gefunden 
			...
			{	
				...
				System.out.print( "Bitte geben Sie eine Zahl ein, durch die die gesuchte Zahl ersetzt werden soll: " );
				ersetz = Integer.parseInt( inData.readLine() );
				
				for(index = 0; index < data.length; index++) {
					
					if(such == data[index]) {
						data[index]= ersetz;
						System.out.println("Die gesuchte Zahl wurde am Index "+ index + " = Position " + (index+1) + " gefunden und durch " + ersetz + " ersetz");
						gefunden = true;
					}
				}
			}
